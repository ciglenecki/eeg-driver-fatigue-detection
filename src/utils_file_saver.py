...
def save_to_file(
    ...
    filename = "-".join([name, timestamp, metadata_str + extension])[:255]
    file_path = str(Path(dir, filename))

    file_saver(obj, file_path)
    os.setxattr(file_path, METADATA_FIELD_TAGS, metadata_bytes)

    print("Saved file:", str(file_path))


# def save_model(model, model_name, score, dir: Path, metadata={}, prefix=""):
#     name = "{model_name}_{score:.4f}_{prefix}_{timestamp}_".format(model_name=model_name, prefix=prefix, score=score).lower()
#     save_to_file(model, dir, name, ".model", lambda model, filename: dump(pickle.dumps(model), filename), metadata=metadata)
