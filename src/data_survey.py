...
parser.add_argument('--epoch_elems', metavar='N', type=int,
                    help='Duration of signal (<= 1000)')
...
def get_epochs_from_signal(filename: str):
    eeg = read_raw_cnt(
        filename, eog=['HEOL', "HEOR", "VEOU", "VEOL"], preload=True, verbose=False)
    eeg_filtered = eeg.notch_filter(
...
for pair in user_state_pairs:
    ...
    for i_poch in range(0, SIGNAL_REQUESTED_SECONDS):
        ...
        df_sample_entropy = df_electrodes.apply(
            func=lambda x: pd_sample_entropy(x, standardize_input=True), axis=0)  # type: ignore
        df_fuzzy_entropy = df_electrodes.apply(
            func=lambda x: pd_fuzzy_entropy(x, standardize_input=True), axis=0)  # type: ignore
